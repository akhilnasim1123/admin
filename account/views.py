                    ...
                    if guestItem:
                        ...
                        for item in items:
                            ...
                            check = OrderItems.objects.filter(
                                account=user, product=item.product).exists()
                            if check:
                                check = OrderItems.objects.filter(
                                    account=user, product=item.product)
                                for ch in check:
                                    ch.quantity = ch.quantity + item.quantity
                                    ch.save()
                            else:
                                OrderItems.objects.create(
                                    account=user, product=item.product, quantity=item.quantity, order=item.order)
                                item.delete()
                        items.delete()
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            wishlist = Wishlist.objects.filter(
                product=val, account=customer).exists()
            if wishlist:
                wishlist = wishlist
